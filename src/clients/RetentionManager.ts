...
export class RetentionDeletionError extends RetentionError {
  constructor(message: string, public readonly key: string, cause?: Error) {
  ...
  async cleanupExpiredBackups(prefix: string): Promise<RetentionResult> {
    ...
    try {
      ...
      try {
        ...
        const listingError = new RetentionListingError(`Failed to list backups for cleanup: ${this.formatError(error)}`, error instanceof Error ? error : undefined);
      ...
      const deletionPromises = objects.map(async (obj) => {
        try {
          if (this.isBackupExpired(obj.key, obj.lastModified)) {
            ...
            console.log(`Deleted expired backup: ${obj.key} (created: ${obj.lastModified.toISOString()})`);
          } else {
            console.debug(`Keeping backup: ${obj.key} (created: ${obj.lastModified.toISOString()})`);
          }
        } catch (error) {
      ...
      const successRate = result.totalCount > 0 ? ((result.totalCount - result.errors.length) / result.totalCount * 100).toFixed(1) : '100';
      console.log(`Retention cleanup completed: ${result.deletedCount} backups deleted out of ${result.totalCount} total (${successRate}% success rate)`);
      
      if (result.errors.length > 0) {
        console.warn(`Retention cleanup had ${result.errors.length} errors. Some backups may not have been deleted.`);
      }
      
    } catch (error) {
      const generalError = new RetentionError(`Unexpected error during retention cleanup: ${this.formatError(error)}`, 'cleanup', error instanceof Error ? error : undefined);
