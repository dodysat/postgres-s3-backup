import {
  RetentionManager,
  RetentionError,
  RetentionListingError,
  RetentionDeletionError,
} from '../src/clients/RetentionManager';
...
describe('RetentionManager', () => {
  ...
  describe('isBackupExpired', () => {
    ...
    it('should handle retention boundary plus one day', () => {
      ...
      const result = retentionManager.isBackupExpired(
        'past-boundary-backup.sql.gz',
        pastBoundaryDate
      );
  ...
  describe('cleanupExpiredBackups', () => {
    ...
    it('should skip cleanup when no retention policy is configured', async () => {
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith(
        'No retention policy configured, keeping all backups'
      );
    });
    ...
    it('should handle S3 listing errors', async () => {
      ...
      expect(mockConsoleError).toHaveBeenCalledWith(
        'Failed to list backups for cleanup: Network error'
      );
    });
  ...
  describe('enhanced error handling and recovery', () => {
    ...
    it('should retry individual deletion operations on transient failures', async () => {
      ...
      mockS3Client.listObjects.mockResolvedValue(mockObjects);
      mockS3Client.deleteObject.mockRejectedValueOnce(transientError).mockResolvedValueOnce();
    ...
    it('should log retry attempts with exponential backoff', async () => {
      ...
      expect(mockConsoleWarn).toHaveBeenCalledWith(
        expect.stringContaining(
          'Attempt 1 failed for list S3 objects for retention cleanup: Error: Network error. Retrying in 1000ms...'
        )
      );
      expect(mockConsoleWarn).toHaveBeenCalledWith(
        expect.stringContaining(
          'Attempt 2 failed for list S3 objects for retention cleanup: Error: Network error. Retrying in 2000ms...'
        )
      );
    });
    ...
    it('should calculate and log success rate', async () => {
      ...
      expect(mockConsoleLog).toHaveBeenCalledWith(expect.stringContaining('(66.7% success rate)'));
      expect(mockConsoleWarn).toHaveBeenCalledWith(
    ...
    it('should handle unexpected errors during cleanup', async () => {
      ...
      expect(result.errors).toContain(
        'Unexpected error during retention cleanup: Error: Unexpected system error'
      );
      expect(mockConsoleError).toHaveBeenCalledWith(
        'Retention cleanup stack trace:',
        expect.any(String)
      );
    });
  ...
  describe('edge cases and error handling', () => {
    ...
    it('should handle deletion with non-Error objects', async () => {
      ...
      expect(result.errors).toContain(
        'Failed to delete backup backups/postgres-backup-2023-12-10_10-00-00.sql.gz: String deletion error'
      );
    });
